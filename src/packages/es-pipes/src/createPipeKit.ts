...
import { LATEST_STREAM_INSTRUCTION_TYPE } from './entities';
import { LEADING_STREAM_INSTRUCTION_TYPE } from './entities';
import { ONCE_STREAM_INSTRUCTION_TYPE } from './entities';
...
export function createPipeKit(createFill: CreateFill, adjuncts: Adjunct[]): PipeKit {
  ...
  const handleEmitData = (streamGroup: StreamGroup, dataBarrel: DataBarrel): void => {
    ...
    const workingPipe = dataBarrel.dataType === dataType.error ? pipeState.errorPipe : pipeState.dataPipe;
